...
def train_clean (train_loader, device,optimizer,model,CE_loss, lr_schedule, epoch_i, transform_rt):
    a = 0
    print(transform_rt)
...
def val(val_loader, device, model,\
        correct_final_nat, best_acc_nat, correct_final_rt, best_acc_rt, checkpoint_loc, transform_rt):
    ...
    for images_1, labels in tqdm(val_loader):
        ...
        # correct_final.append(correct)
        correct_final_rt = correct_final_rt + correct_2

    acc_rt = correct_final_rt / len(val_loader.dataset)
    ...
    if (acc_nat+acc_rt)/2 > best_avg:
            ...
            best_acc_rt = acc_rt
            best_avg = (best_acc_nat+best_acc_rt)/2
            # best_acc_l2 = acc_l2
            # best_acc_linf = acc_linf
    
    return acc_nat, best_acc_nat, acc_rt, best_acc_rt,  best_avg
...
def train(args):
    ...
    for i in range(args.epochs):
        ...
        train_clean (train_loader, device,optimizer,model,CE_loss, lr_schedule, i, transform_rt)
       ...
       # train_clean (train_loader, device,optimizer,model,CE_loss)
        ...
        acc_nat, best_acc_nat, acc_rt, best_acc_rt, acc_avg = val(test_loader, device, model,\
        correct_final_nat, best_acc_nat, correct_final_rt, best_acc_rt, \
            args.checkpoint_loc, transform_rt)
        ...
        print('Epoch: ', i+1, ' Done!!  rotaion  Accuracy: ', acc_rt)
        print('Epoch: ', i+1, '  Best rotaton  Accuracy: ', best_acc_rt)

        print('Epoch: ', i+1, '  Best avg  Accuracy: ', best_acc_rt)
