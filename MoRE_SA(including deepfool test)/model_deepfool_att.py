...
def deepfool(image, net, num_classes=10, overshoot=0.02, max_iter=100, norm="2", eps=1.0):
    """
    ...
    while k_i == label and loop_i < max_iter:
        ...
        if norm == '2':
            ...
            # r_i = abs(pert + 1e-4) * np.sign(w) / np.linalg.norm(w.flatten(), ord=1)
            r_i = abs(pert + 1e-4) * np.sign(w)
        else:
            raise Exception("norm not specified!")
