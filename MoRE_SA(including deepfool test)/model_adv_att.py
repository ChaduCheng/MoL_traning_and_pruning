...
class AttackPGD_expert(nn.Module):  # change here to build l_2 and l_inf
    def __init__(self, basic_net, config):
        super(AttackPGD_expert, self).__init__()
        self.basic_net = basic_net
        self.rand = config['random_start']
        self.step_size = config['step_size']
        self.epsilon = config['epsilon']
...
def deepfool(image, net, num_classes=10, overshoot=0.02, max_iter=10):
    ...
    im_first = Variable(image[:, :, :], requires_grad=True)
    f_image = net.forward(im_first).data.cpu().numpy().flatten()
    ...
    while k_i == label and loop_i < max_iter:
        ...
        r_i =  (pert+1e-6) * w / np.linalg.norm(w)
