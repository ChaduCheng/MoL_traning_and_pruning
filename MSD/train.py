...
#
# transform = utils.get_transformation('cifar')
# transform_rt = utils.get_transformation_rotation('cifar')
# dataset_new = utils.get_dataset('cifar', transform, 0.02)
#
# # operate this train_loader to generate new loader
#
# train_loader = DataLoader(dataset_new['train_data'], batch_size=128, shuffle=True)
# # val_loader = DataLoader(dataset['val_data'], batch_size = args.batch_size, shuffle=True)
# test_loader = DataLoader(dataset_new['test_data'], batch_size=128, shuffle=True)

output_classes = 10

transform = utils.get_transformation('cifar')
dataset = utils.get_dataset('cifar', transform, None)
...
for epoch_i in tqdm(range(1,epochs+1)):
    ...
    if epoch_i %5 == 0:
        torch.save(model.state_dict(), "{0}/all_iter_{1}.pt".format(model_dir, str(epoch_i)))
