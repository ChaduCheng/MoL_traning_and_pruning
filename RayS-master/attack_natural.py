...
def main():
    ...
    #     torch_model = GeneralTorchModel(model, n_class=10, im_mean=None, im_std=None)
    if args.dataset == 'cifar':
        model = MoE_ResNet18_test(5, 10).cuda()
        model = torch.nn.DataParallel(model, device_ids=[0])
        # checkpoint = torch.load('../checkpoint/ckpt_resnet18_cifar_linf_8_255.pth')
        checkpoint = torch.load(
            '/proj/usr/hao.cheng/test/cifar_16_resenet/checkpoint/ckptMoE_resnet_cifar_clean+4adv_ensure_ori_model_no_clean_epoch.pth')
        model.load_state_dict(checkpoint['net'])
        test_loader = load_cifar10_test_data(args.batch)
        torch_model = GeneralTorchModel(model, n_class=10, im_mean=None, im_std=None)
